<?php
class SAdmin_m extends CI_Model {
	...
	public function getSelectedCategory($param){

	    $this->db->select('*');
		$this->db->from('category cat');
		$this->db->where('cat.category_id',$param["category_id"]);           
		$results = $this->db->get();
		if($results->num_rows() > 0){
			return $results->result_array();
		}
	...
	public function selected_sd($params){
	
	$this->db->select('*');
	$this->db->from('sd sds'); 
	$this->db->where('sds.sd_id',$params["sd_id"]);   
	...
	public function check_sd_exist($params){
	
	$this->db->select('*');
	$this->db->from('sd sds'); 
	$this->db->where('sds.sd_name',$params["sd_name"]);    
	$this->db->or_where('sds.sd_email',$params["sd_email"]);  
	$results = $this->db->get();
	if($results->num_rows() > 0){
	     return true;
		}else{
	...
	public function check_editedsd_exist($params){
    $sd_id = $params['sd_id'];
	$sd_name = $params['sd_name'];
	$sd_email = $params['sd_email'];
    $result = $this->db->query("SELECT * FROM sd WHERE sd_id != '$sd_id' AND sd_name = '$sd_name' OR sd_id != '$sd_id' AND sd_email = '$sd_email' ");
	if($result->num_rows() > 0){		
	   return true;
	 }
	 else {
	...
	public function selected_tm($params){
	
	$this->db->select('*');
	$this->db->from('tm tms'); 
	$this->db->where('tms.tm_id',$params["tm_id"]);   
	...
	public function check_TM_exist($params){
	
	$this->db->select('*');
	$this->db->from('tm tms'); 
	$this->db->where('tms.tm_name',$params["tm_name"]);    
	$this->db->or_where('tms.tm_email',$params["tm_email"]);  
	$results = $this->db->get();
	if($results->num_rows() > 0){
	     return true;
		}else{
	...
	public function check_editedtm_exist($params){
    $tm_id = $params['tm_id'];
	$tm_name = $params['tm_name'];
	$tm_email = $params['tm_email'];
    $result = $this->db->query("SELECT * FROM tm WHERE tm_id != '$tm_id' AND tm_name = '$tm_name' OR tm_id != '$tm_id' AND tm_email = '$tm_email' ");
	if($result->num_rows() > 0){		
	   return true;
	 }
	 else {
	...
	public function selected_cm($params){
	
	$this->db->select('*');
	$this->db->from('cm cms'); 
	$this->db->where('cms.cm_id',$params["cm_id"]);   
	...
	public function check_CM_exist($params){
	
	$this->db->select('*');
	$this->db->from('cm cms'); 
	$this->db->where('cms.cm_name',$params["cm_name"]);    
	$this->db->or_where('cms.cm_email',$params["cm_email"]);  
	$results = $this->db->get();
	if($results->num_rows() > 0){
	     return true;
		}else{
	...
	public function check_editedcm_exist($params){
    $cm_id = $params['cm_id'];
	$cm_name = $params['cm_name'];
	$cm_email = $params['cm_email'];
    $result = $this->db->query("SELECT * FROM cm WHERE cm_id != '$cm_id' AND cm_name = '$cm_name' OR cm_id != '$cm_id' AND cm_email = '$cm_email' ");
	if($result->num_rows() > 0){		
	   return true;
	 }
	 else {
	...
	$emp_adpId = $params['emp_adpId'];
	 $results = $this->db->query("SELECT * FROM employee WHERE emp_name = '$emp_name' OR emp_email = '$emp_email' ");
	if($results->num_rows() > 0){
	     return true;
		}else{
		return false;
	...
	$emp_email = $params['emp_email'];
	$emp_adpId = $params['emp_adpId'];
    $result = $this->db->query("SELECT * FROM employee WHERE emp_id != '$emp_id' AND emp_name = '$emp_name' OR emp_id != '$emp_id' AND emp_email = '$emp_email' ");
	if($result->num_rows() > 0){		
	   return true;
	 }
	 else {
	...
	public function getOrders(){
		...
		$this->db->join('roles role', 'role.role_id = emp.role_id', 'INNER');
		$this->db->join('category cate', 'cate.category_id = order.category_id', 'INNER');
	...
	public function exportOrders(){
	    $this->db->select('orderdetails_id , order_date , store_name , store_uId , product_name, product_unit , product_code , orderdetails_availableStock ,  orderdetails_requiredItems , financestatus_name , status_name ');
		$this->db->from('orderdetails orderdetail');
		$this->db->join('orders order', 'order.order_id = orderdetail.order_id','INNER');
		$this->db->join('status sts', 'sts.status_id = order.status_id', 'INNER');
		$this->db->join('product prd', 'prd.product_id = orderdetail.product_id','INNER');
