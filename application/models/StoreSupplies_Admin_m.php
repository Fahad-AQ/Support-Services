<?php
class StoreSupplies_Admin_m extends CI_Model {
	...
	public function exportOrders(){
	    $this->db->select('orderdetails_id , order_date ,order_refNo , store_name , store_uId , product_name, product_unit , product_code , orderdetails_availableStock ,  orderdetails_requiredItems , orderdetails_comment , financestatus_name , status_name , order_finalStatusComment');
		$this->db->from('orderdetails orderdetail');
		$this->db->join('orders order', 'order.order_id = orderdetail.order_id','INNER');
		$this->db->join('status sts', 'sts.status_id = order.status_id', 'INNER');
		$this->db->join('product prd', 'prd.product_id = orderdetail.product_id','INNER');
