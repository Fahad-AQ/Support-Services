        ...
        <div class="row">
          <div class="col-lg-12">
            <section class="panel default blue_title h2">
              <div class="panel-heading">Add CM</div>
              <div class="panel-body">
                <form id="add_CM">
                    <div class="form-group">
                      <label for="name">CM Name:</label>
                      <input type="text" name="cm_name"  class="form-control border-info required_colom" required="required" id="cm_name">
                    </div>
                     <div class="form-group">
                      <label for="name">CM Email:</label>
                      <input type="email" name="cm_email"  class="form-control border-info required_colom" required="required" id="cm_email">
                    </div>
...
<script>
  $('#msg2').hide();
  var add_CM = $('#add_CM');  
    add_CM.on('submit',function(e){
      e.preventDefault();
      
      var new_prd_add = add_CM.serialize();
      ...
      if(assetserror > 0){return false;}else{
        $.ajax({
          type:'POST',
          url:'<?php echo base_url('sAdmin/add_CMForm'); ?>',
          data:new_prd_add,
          dataType:'JSON',
          success:function(data){ 
            if(data.status == 'CM Already Exist'){
              ...
              $('.msg').show().html('CM Added Succesfully').hide(2500);
               add_CM[0].reset();
