        ...
        <div class="row">
          <div class="col-lg-12">
            <section class="panel default blue_title h2">
              <div class="panel-heading">Add Market</div>
              <div class="panel-body">
                <form id="add_market">
                    <div class="form-group">
                      <label for="name">Market Name:</label>
                      <input type="text" name="market_name" class="form-control border-info required_colom" required="required" id="name">
                    </div>
                      <!-- <div class="form-group">
...
<script>
  $('#msg2').hide();
  var add_market = $('#add_market');  
    add_market.on('submit',function(e){
      e.preventDefault();
      
      var new_prd_add = add_market.serialize();
      ...
      if(assetserror > 0){return false;}else{
        $.ajax({
          type:'POST',
          url:'<?php echo base_url('sAdmin/add_marketForm'); ?>',
          data:new_prd_add,
          dataType:'JSON',
          success:function(data){ 
            if(data.status == 'Market Already Exist'){
              ...
              $('.msg').show().html('Market Added Succesfully').hide(2500);
               add_market[0].reset();
                setTimeout(function(){location.href="<?php echo base_url('sAdmin/marketList'); ?>"} , 2500);  
            }
          },
