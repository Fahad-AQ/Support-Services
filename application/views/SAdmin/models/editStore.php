                   ...
                   <div class="form-group">
                        <label for="activeStatus">Selected Market:</label>
                          ...
                          <?php foreach ($markets as $key => $market) {?>
                                 <option  <?php if($selected_stores[0]["market_id"] == $market["market_id"]){echo 'selected';}?> value="<?php echo $market["market_id"];?>"><?php echo $market["market_name"];?></option>
                       <?php }; ?>
                   ...
                   <div class="form-group">
                        <label for="activeStatus">Selected SD:</label>
                          ...
                          <?php foreach ($sds as $key => $sd) {?>
                                 <option  <?php if($selected_stores[0]["sd_id"] == $sd["sd_id"]){echo 'selected';}?> value="<?php echo $sd["sd_id"];?>"><?php echo $sd["sd_name"];?></option>
                       <?php }; ?>
                   ...
                   <div class="form-group">
                        <label for="activeStatus">Selected TM:</label>
                          ...
                          <?php foreach ($tms as $key => $tm) {?>
                                 <option  <?php if($selected_stores[0]["tm_id"] == $tm["tm_id"]){echo 'selected';}?> value="<?php echo $tm["tm_id"];?>"><?php echo $tm["tm_name"];?></option>
                       <?php }; ?>
                   ...
                   <div class="form-group">
                        <label for="activeStatus">Selected CM:</label>
                          ...
                          <?php foreach ($cms as $key => $cm) {?>
                                 <option  <?php if($selected_stores[0]["cm_id"] == $cm["cm_id"]){echo 'selected';}?> value="<?php echo $cm["cm_id"];?>"><?php echo $cm["cm_name"];?></option>
                       <?php }; ?>
  ...
  var edit_store = $('#edit_store');  
    edit_store.on('submit',function(e){
      ...
      if(assetserror > 0){return false;}else{
        $.ajax({
          type:'POST',
          url:'<?php echo base_url('sAdmin/editStore'); ?>',
          data:new_prd_add,
          dataType:'JSON',
