                  ...
                  <div class="form-group">
                        <label for="activeStatus">Selected Store:</label>
                          ...
                          <?php foreach ($stores as $key => $store) {?>
                                 <option  <?php if($selected_employees[0]["store_id"] == $store["store_id"]){echo 'selected';}?> value="<?php echo $store["store_id"];?>"><?php echo $store["store_name"];?></option>
                       <?php }; ?>
                  ...
                  <div class="form-group">
                        <label for="activeStatus">Selected Role:</label>
                          ...
                          <?php foreach ($roles as $key => $role) {?>
                                 <option  <?php if($selected_employees[0]["role_id"] == $role["role_id"]){echo 'selected';}?> value="<?php echo $role["role_id"];?>"><?php echo $role["role_name"];?></option>
                       <?php }; ?>
                   ...
                   <div class="form-group">
                        <label for="activeStatus">Employee Status:</label>
                          <select class="form-control" id="activeStatus" name="emp_type" >
                            <option  <?php if($selected_employees[0]["emp_type"] == 1){echo 'selected';}?> value="1" >Corporate Office</option>
                            <option  <?php if($selected_employees[0]["emp_type"] == 2){echo 'selected';}?> value="2">Back Office</option>
                          </select>
                   </div>
  ...
  var edit_employee = $('#edit_employee');  
    edit_employee.on('submit',function(e){
      ...
      if(assetserror > 0){return false;}else{
        $.ajax({
          type:'POST',
          url:'<?php echo base_url('sAdmin/editEmployee'); ?>',
          data:new_prd_add,
          dataType:'JSON',
            ...
            if(data.status == 'Employee Already Exist'){
              ...
              $('.msg').show().html('Employee Edited Succesfully').hide(2500);
               edit_employee[0].reset();
                setTimeout(function(){location.href="<?php echo base_url('sAdmin/employeeList'); ?>"} , 2500);  
            }
          },
