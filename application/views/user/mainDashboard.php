...
<div class="wrapper">
  ...
  <div class="login_page">
      <div class="login_content">
           ...
           <div class="msg alert alert-styled-left"></div>
          ...
          <?php echo form_open(base_url(''),array('class'=>"form-horizontal", 'id'=>'selectCategory')); ?>
              <div class="form-group">
                 <select class="form-control required_colom" id="activeStatus" name="category_id">
                        <option  value="">Select Category</option>
                        <?php foreach ($categorys as $key => $category) {if( $category["category_status"] == 1 && $category["category_id"] != 2 || $category["category_status"] == 1 && $category["category_id"] == 2 && $this->session->userdata('empType') == 2 ){?>
    ...
    selectCategory.on('submit',function(e){
      ...
      var assetserror = 0;
          ...
          $('.required_colom').each(function(){
            if($(this).val() == ''){$(this).addClass('redborder1');assetserror = assetserror + 1;

              $('.msg').show();
              $('.msg').removeClass('alert-success');
              $('.msg').removeClass('alert-danger');
              $('.msg').addClass('alert-info');
              $('.msg').show().html('Please Select Category');


              }
             else{

              $(this).removeClass('redborder1'); 
      ...
      if(assetserror > 0){return false;}else{
           ...
           $('.msg').hide();
            ...
            $.ajax({
            type:'POST',
            url:'<?php echo base_url('user/setCategory'); ?>',
            data:new_prd_add,
            dataType:'JSON',
            success:function(data){ 
              if(data.status == 'selected'){
                ...
                $('.msg').show().html('Selected : ' +data.selected["category_name"]);
                 setTimeout(function(){location.href="<?php echo base_url('user'); ?>"} , 500 );
              }
            },
