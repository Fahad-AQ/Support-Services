...
<div class="row">
          ...
          <tr>
            ...
            <td><input type="number"  step='any' name="avl[]"  min="0" value="<?php echo $selected_order['orderdetails_availableStock'];?>" placeholder="Type Available Stock" class="form-control border-info required_colom" required="required">&nbsp;PER&nbsp;
            <?php echo substr($selected_order['product_unit'], strpos($selected_order['product_unit'], "-") + 1)?>
            </td>
            <td><input type="number"  step='any' name="rmt[]"  value="<?php echo $selected_order['orderdetails_requiredItems'];?>" min="0" placeholder="Type Required Stock" class="form-control border-info required_colom" required="required">&nbsp;PER&nbsp;
            <?php echo substr($selected_order['product_unit'], strpos($selected_order['product_unit'], "-") + 1)?>
            </td>
             <td><input type="text"  step='any' name="cmt[]"  value="<?php echo $selected_order['orderdetails_comment'];?>" placeholder="Type Comment" class="form-control border-info">
            </td>
          </tr> 
          <?php 
          } ?>  
  ...
  var edit_product = $('#edit_product');  
    edit_product.on('submit',function(e){
      ...
      if(assetserror > 0){return false;}else{
        $.ajax({
          type:'POST',
          url:'<?php echo base_url('storeSupplies/user/storeSupplies_editOrder'); ?>',
          data:new_prd_add,
          dataType:'JSON',
